---
title: "INFSCI 2595 Spring 2024 Homework: 07"
subtitle: "Assigned March 11, 2024; Due: March 22, 2024"
author: "Bhavana Devulapally"
date: "Submission time: March 21, 2024 at 11:00PM EST"
...
The dimension of Xmat_quad_small is [5, 3], indicating that it has 5 rows and 3 columns.
The dimension of Xmat_quad_big is [30, 3], indicating that it has 30 rows and 3 columns.

**Display the entire small data design matrix to the screen. What values are contained in the FIRST column and why?**  

In the FIRST column of the design matrix, all the values are 1. This column represents the intercept term in the regression model. The intercept term allows the regression line to intersect the y-axis at a specific point, even if the input variable (x) is zero. Therefore, in regression analysis, the intercept term is always set to 1 to capture the constant term in the model equation. Consequently, the values contained in the FIRST column of the design matrix are all 1.
...
```{r,solution_1e}
covmat_quad_small_noprior %>% dim()
covmat_quad_big_noprior %>% dim()
```

**How many rows and columns does EACH posterior covariance matrix have?** 

Based on the above outputs we can say that,

=> The posterior covariance matrix `covmat_quad_small_noprior` has dimensions `[3, 3]`, indicating that it has 3 rows and 3 columns.
=> The posterior covariance matrix `covmat_quad_big_noprior` has dimensions `[3, 3]`, indicating that it also has 3 rows and 3 columns.

Therefore, both the small and big posterior covariance matrices have 3 rows and 3 columns.
...
```
The above query focuses particularly on the quadratic aspect, offering scaled posterior standard deviations for all three coefficients. Notably, the standard deviation pertaining to the quadratic attribute is mentioned last, around 1.3 on a scaled basis. This implies that the posterior 95% uncertainty range would spread approximately ±1.3 times around the posterior mean.


The scaled posterior standard deviations obtained from the "big" dataset are given above. Notably, the scaled posterior standard deviation associated with the quadratic feature is roughly 10 times smaller in comparison to the scenario observed with the small dataset. This suggests that the "big" dataset offers notably higher precision relative to the scenario involving only 5 measurements.
...
postmeans_quad_small_noprior <- solve(crossprod(Xmat_quad_small), crossprod(Xmat_quad_small, y_quad_small_col))
```


```{r, solution_for_1g_b, eval=TRUE}
### posterior mean vector for the BIG data
postmeans_quad_big_noprior <- solve(crossprod(Xmat_quad_big), crossprod(Xmat_quad_big, y_quad_big_col))
```


Display the posterior means and answer the conceptual question.  

The posterior mean for the small data case is given by:
```{r}
postmeans_quad_small_noprior %>% as.vector()
```
The posterior mean for the big data case is given by:
```{r}
postmeans_quad_big_noprior %>% as.vector()
```
No, the values of the posterior means would remain the same regardless of the value of sigma(σ). This is because the posterior mean calculations are based on the assumption that the prior does not matter, implying an infinitely diffuse prior or a prior with infinite uncertainty. Therefore, the choice of sigma does not affect the posterior mean values.
...
tibble::tibble(
  coef_name = colnames(Xmat_quad_small),
  post_mean = as.vector(postmeans_quad_small_noprior),
  ...
  mutate(significant_feature = sign(post_lwr) == sign(post_upr))



tibble::tibble(
  coef_name = colnames(Xmat_quad_big),
  post_mean = as.vector(postmeans_quad_big_noprior),
  post_lwr = as.vector(postmeans_quad_big_noprior - 2*sqrt(diag(covmat_quad_big_noprior))),
  post_upr = as.vector(postmeans_quad_big_noprior + 2*sqrt(diag(covmat_quad_big_noprior)))
) %>% 
  mutate(significant_feature = sign(post_lwr) == sign(post_upr))
...
From the above outputs in both datasets, the quadratic feature is deemed statistically significant since the 95% posterior uncertainty intervals exclude zero. However, there's a notable contrast in interpretation between the two cases: in the small data scenario, significance is inferred in a positive direction, whereas in the big data scenario, it's inferred in a negative direction. This variation in interpretation introduces an intriguing aspect to the analysis.
...
Below is the covariance matrix for the vague prior:
...
Below is the covariance matrix for the informative prior:
...
The above code shows the posterior standard deviations for all three features derived from the small data case. Specifically, only the standard deviation of the quadratic feature's coefficient is shown.
...
The outcome from the small data analysis with the informative prior differs from the posterior uncertainty observed when no prior is applied to the small data. This indicates that the informative prior constrained the permissible range of the coefficients, leading to a reduction in uncertainty.


```{r}
covmat_quad_big_inform %>% diag() %>% sqrt()
```
Above code is the posterior standard deviations derived from the informative prior and the "big" data case.

```{r}
covmat_quad_big_noprior %>% diag() %>% sqrt()
```
The posterior standard deviation of the quadratic feature associated with the "big" data set is comparable to that of the no-prior case in the above code. By this we can say that the data likely dominated the influence of the prior. The result from the no-prior case is given above.



The posterior standard deviations calculated based on the informative prior are similar to the results from 1f) for the big data case, but there are slight differences for the small data case.

For the big data case, the posterior standard deviation calculated here is approximately 0.1545, which is quite close to the value of approximately 0.1566 obtained in 1f). This suggests that the posterior standard deviation is consistent across both approaches for the big data case.

However, for the small data case, the posterior standard deviation calculated here is approximately 0.8848, while the value obtained in 1f) was approximately 1.3021. Although there is some difference between these values, they are in the same order of magnitude. This difference may arise due to the influence of the prior distribution and the size of the dataset.

Overall, while there may be slight differences, the posterior standard deviations calculated here show similar trends to the results from 1f) for both data cases.
...
```
Above code is the result for the small data case.

```{r}
covmat_quad_big_vague %>% diag() %>% sqrt()
```
The result for the "big" data case is displayed above.

The vague prior, indicated by a prior standard deviation of 30, places minimal constraints on the coefficient values. With a prior 95% uncertainty interval ranging from -60 to +60, the prior behaves similarly to having no prior at all. As a result, the posterior standard deviations obtained from both the small and big data cases closely resemble those obtained without any prior assumptions.



For the small data case: The posterior standard deviation calculated here (1.300423) is very close to the value obtained in 1f) (approximately 1.3021). This suggests that the posterior standard deviation is consistent across both approaches for the small data case.


For the big data case: The posterior standard deviation calculated here (0.1566258) is almost identical to the value obtained in 1f) (approximately 0.1566). This indicates that the posterior standard deviation is consistent across both approaches for the big data case as well.

Hence, the posterior standard deviations calculated based on the vague prior are very similar to the results from 1f) for both data cases.
...
The prior helps to focus attention on a specific range of values, with the width of this range determined by the prior's standard deviation. A larger standard deviation allows for a wider range of values to be considered, while a smaller standard deviation limits the range of possible coefficients. Therefore, in situations with limited data, narrowing down meaningful intervals with smaller standard deviations can help decrease uncertainty in the posterior distribution.
...
lm_logpost <- function(unknowns, my_info)
{
  ...
  beta_v <- as.vector(unknowns[1:length_beta])
  ...
  X <- as.matrix(my_info$design_matrix)
...
No, We don't need to worry about the initial guess's impact because the log-posterior surface of a linear model is unimodal.
...
There are 30 rows representing predictions for 30 observations, while the 2500 columns signify the generation of 2500 posterior samples for the unknown parameters.
...
nrow(Xmat_quad_big)
```
**What is the length of the `error_quad_post_01` vector?**
The length of the `error_quad_post_01` vector is 30.
...
signif(c(mae_quad_post_01, mae_quad_post_02, mae_quad_post_03), 6)
```

? 
**What are you averaging over when you calculate the mean absolute error? Are the three MAE values the same? If not, why would they be different?**  

Although the three Mean Absolute Errors (MAEs) show similarities, they are not identical. The mean trend consistently follows a predictable pattern based on specified inputs and β-parameters. However, due to variations in the β-parameters across posterior samples, different intercepts and slopes may result, even when calculating means at the same input values in each sample.
...
The columns of the `absError_quad_mat` represent individual posterior samples. To calculate the average for each column, we will use the `colMeans()` function. The data type and length of `MAE_quad` indicate that it is a numeric vector with 2500 elements. Therefore, the length of `MAE_quad` corresponds to the number of previously generated posterior samples.
...
The UNCERTAINITY in the Mean Absolute Error (MAE) is attributed to the uncertainty surrounding the mean trend, which is influenced by the uncertainty in the β parameters. As a result, the variability in intercepts and slopes also contributes to the uncertainty of the MAE.
...
make_spline_basis_mats <- function(J, train_data, test_data)
{
  ...
  train_matrix <- model.matrix(~ splines::ns(x, knots = interior_knots, Boundary.knots = boundary_knots) , data = train_data)
  
  test_matrix <- model.matrix(~ splines::ns(x, knots = interior_knots, Boundary.knots = boundary_knots), data = test_data)
...
By looking at the above output we can say that, the training matrices contain 150 rows, and the test matrices contain 50 rows, matching the row counts of the `train_df` and `test_df` datasets.
...
When considering only the training set, we observe a consistent decrease in the median MAE as the degrees-of-freedom increase, indicating improved accuracy with more complex models relative to the training data. However, for the test set, the median MAE tends to stabilize around 10 degrees-of-freedom. 

The boxplots demonstrate a notable reduction in MAE between 3 and 7 degrees-of-freedom, followed by fluctuations thereafter. Due to the uncertainty surrounding MAE, determining the best-performing model is challenging. Models with degrees of freedom between 10 and 30 exhibit similar effectiveness, suggesting that opting for simplicity among them is reasonable given their comparable performance.
...
Based on the visualization of posterior model weights above, the model with 7 degrees of freedom appears to be the best.
...
Evaluating model performance based solely on training set error metrics suggests that the most complex model performs the best. However, when comparing performance on the test dataset, we observe that error levels off around 10 degrees of freedom. Beyond 25 degrees of freedom, increasing complexity doesn't enhance test set performance and may even degrade it. Selecting the best model becomes challenging due to overlap in median MAE and uncertainty among models with 11 through 25 degrees of freedom. In such cases, opting for the simplest model is prudent, leading to the selection of the 7th degree-of-freedom natural spline. This approach is in line with Evidence-based assessment, which integrates over all allowable parameter combinations to distinguish between good and poor fits. The Laplace Approximation penalizes the addition of more terms to the model, as reflected in the log-determinant of the Hessian matrix. Therefore, simpler models like the 7 and 9 degrees-of-freedom splines, with fewer parameters to learn, incur lower complexity penalties compared to higher degree models. Additional parameters beyond these levels may reduce training set error but contribute little to improved model performance and may begin to interpolate noise.
